...
import pytest
from ble_mcp.safety import SafetyTier, classify_tool


class TestClassifyTool:
    def test_read_only_tools(self):
        for tool in ["scan_devices", "monitor_advertisements",
                     "enumerate_services", "read_characteristic"]:
            assert classify_tool(tool) == SafetyTier.READ_ONLY

    def test_allowed_write_tools(self):
        for tool in ["connect", "disconnect", "subscribe_notify"]:
    ...
    def test_no_approval_write_tools_in_mvp(self):
        ...
        from ble_mcp.safety import _TOOL_TIERS
        approval_tools = [
